...
// --- CẤU HÌNH THÍ NGHIỆM ---
const float alpha = 0.3; // Đổi thành 0.1, 0.3, 0.7 theo yêu cầu Bảng 5
const unsigned long SAMPLE_INTERVAL_US = 20000; // Tần số lấy mẫu 50Hz (20,000 microseconds)
...
void setup() {
  ...
  // Cấu hình tối ưu cho đặc tính động nhanh
...
void loop() {
  ...
  // Chỉ thực thi khi đúng chu kỳ lấy mẫu (Non-blocking)
  if (currentMicros - previousMicros >= SAMPLE_INTERVAL_US) {
    
    // Cộng dồn khoảng thời gian lấy mẫu để CHỐNG TRÔI THỜI GIAN (Drift)
    // Tuyệt đối không dùng: previousMicros = currentMicros;
    ...
    // 3. Tính toán thời gian thực (ms) với độ phân giải cao để vẽ đồ thị
  ...
  // Vòng loop chạy tự do cực nhanh, không có delay nào cản trở
}
